...
const { sendSuceess, setError, sendMessage } = require('../Utils/setMessages');
...
const UserController = {
    ...
    newUser: async (req, res) => {
        ...
        if (!email || !password || !confirmPass) {
            return res.status(404).json(sendMessage('error', 'Mising data try again'));
        }
        if (password !== confirmPass) {
            return res.status(409).json(sendMessage('error', 'Password no confirmed, try again'))
        }
        try {
    ...
    getUser: async (req, res) => {
        ...
        try {
            ...
            if (!resultFind) {
                return res.status(404).json(sendMessage('error', 'User not found try again or contact with the Admin'));
            }
            let result = {};
    ...
    deleteUser: async (req, res) => {
        let { id } = req.params;
        let { deToken } = req.headers;
        if (! await itsMyUser(deToken, id)) return res.status(403).json(sendMessage('error', 'Error of authentication'));
        try {
    ...
    restoreUser: async (req, res) => {
        let { id } = req.params;
        let { deToken } = req.headers;
        if (! await isAdmin(deToken)) return res.status(403).json(sendMessage('error', 'Error of authentication'));
        try {
